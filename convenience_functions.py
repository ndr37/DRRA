...
def plotscans_overlay_lu(autofit):
	for f in autofit:
		plt.plot(autofit[f][4][:len(autofit[f][4].Field)//4].Field,ap.model(autofit[f][3][2][0],autofit[f][4][:len(autofit[f][4].Field)//4].Field,f),
			autofit[f][4][:len(autofit[f][4].Field)//4].Field,autofit[f][4][:len(autofit[f][4].Field)//4].X)
	...
	plt.show()
	for f in autofit:
		plt.plot(autofit[f][4][len(autofit[f][4].Field)//2:len(autofit[f][4].Field)//4*3].Field,ap.model(autofit[f][3][1][0],autofit[f][4][len(autofit[f][4].Field)//2:len(autofit[f][4].Field)//4*3].Field,f),
			autofit[f][4][len(autofit[f][4].Field)//2:len(autofit[f][4].Field)//4*3].Field,autofit[f][4][len(autofit[f][4].Field)//2:len(autofit[f][4].Field)//4*3].X)
	plt.ylabel('Vmix ($\mu$V)')
	plt.xlabel('Field (T)')
	plt.title('Positive field sweep and fit')
	plt.show()
...
def plotMokeTotalFit(totalfit,MOKEdata):
	...
	plt.title('Fit Residuals')
	plt.show()
	sumyfit = ap.vsumsmooth(MOKEdata.MirrorLine,totalfit['x'][0],totalfit['x'][1],totalfit['x'][2],totalfit['x'][4])
...
def plotMokeTotalFit_offset(totalfit,MOKEdata):
	difyfit = ap.vdifsmooth(MOKEdata.MirrorLine,totalfit['x'][0],totalfit['x'][1],totalfit['x'][3],totalfit['x'][4])
	plt.plot(MOKEdata.MirrorLine,MOKEdata.dif*1e6,MOKEdata.MirrorLine,difyfit, '.',lw=6)
	plt.title('MOKE field swap difference')
	plt.xlabel('Position (Volts)')
...
def area_calcs(totalfit, MOKEdata, Ms,mag_thick, deviceR,activeconductivity):
	...
	sumint=ap.sum_area((totalfit['x'][0],totalfit['x'][1],totalfit['x'][2],totalfit['x'][4]),MOKEdata.MirrorLine)
...
def area_calcs_offset(totalfit, MOKEdata, Ms,mag_thick, deviceR,activeconductivity):
	difint=ap.dif_area((totalfit['x'][0],totalfit['x'][1],totalfit['x'][3],totalfit['x'][4]),MOKEdata.MirrorLine)
	sumint=ap.sum_area_offset((totalfit['x'][0],totalfit['x'][1],totalfit['x'][2],totalfit['x'][4],totalfit['x'][5]),MOKEdata.MirrorLine)
	prefactor = ap.spin_Hall_prefactor(Ms,mag_thick,deviceR,activeconductivity)
	print('Dif Area = %f'% difint)
	print('Sum Area = %f'% sumint)
	print('Dif/Sum Ratio = %f'% (difint/sumint))
